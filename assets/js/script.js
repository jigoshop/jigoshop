		...
		if (!not_set) {
			...
			jQuery.post( params.ajax_url, data, function(response) {
				...
				if (response.length > 1) {
					...
					jQuery('div.product .onsale').hide();
					jQuery('div.product').prepend( variation_response.sale_flash );
					jQuery('div.product p.price').html( variation_response.price_html );
					jQuery('.single_variation').html( variation_response.availability_html );
