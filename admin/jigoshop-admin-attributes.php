...
function jigoshop_save_attributes()
{
	...
	if (isset($_POST['add_new_attribute']) && $_POST['add_new_attribute']) {
		...
		if ((empty($attribute_name) && empty($attribute_label)) || empty($attribute_label)) {
			...
			$wpdb->insert($wpdb->prefix."jigoshop_attribute_taxonomies", array(
		...
		if ($edit > 0) {
			...
			$attribute_label = (string)strip_tags(stripslashes($_POST['attribute_label']));
			$wpdb->update($wpdb->prefix."jigoshop_attribute_taxonomies", array(
				'attribute_type' => $attribute_type,
				'attribute_label' => $attribute_label
...
add_action('admin_init', 'jigoshop_save_attributes');
						...
						<form action="admin.php?page=jigoshop_attributes&amp;edit=<?php echo esc_attr($edit); ?>" method="post">
							...
							<div class="form-field">
								...
								<select name="attribute_type" id="attribute_type" style="width: 100%;">
									...
									} ?>><?php _e('Select', 'jigoshop') ?></option>
									<option value="multiselect" <?php if ($att_type == 'multiselect') {
										echo 'selected="selected"';
									} ?>><?php _e('Multiselect', 'jigoshop') ?></option>
									<option value="text" <?php if ($att_type == 'text') {
										echo 'selected="selected"';
									} ?>><?php _e('Text', 'jigoshop') ?></option>
								</select>
							</div>
	...
	<div class="wrap jigoshop">
							...
							foreach ($attribute_taxonomies as $tax) :
								...
								<td><a href="edit-tags.php?taxonomy=pa_<?php echo sanitize_title($tax->attribute_name); ?>&amp;post_type=product"><?php echo esc_html(ucwords($att_title)); ?></a>
									...
									<div class="row-actions"><span class="edit"><a
												href="<?php echo esc_url(add_query_arg('edit', $tax->attribute_id, 'admin.php?page=jigoshop_attributes')); ?>"><?php _e('Edit', 'jigoshop'); ?></a> | </span><span
											class="delete"><a class="delete"
									                      href="<?php echo esc_url(wp_nonce_url(add_query_arg('delete', $tax->attribute_id, 'admin.php?page=jigoshop_attributes'), 'jigoshop-delete-attribute_'.$tax->attribute_id)); ?>"><?php _e('Delete', 'jigoshop'); ?></a></span>
									</div>
								</td>
								...
								<td><a href="edit-tags.php?taxonomy=pa_<?php echo sanitize_title($tax->attribute_name); ?>&amp;post_type=product"
								       class="button alignright"><?php _e('Configure&nbsp;terms', 'jigoshop'); ?></a></td>
								</tr><?php
							endforeach;
						else :
							?>
							<tr>
							<td colspan="5"><?php _e('No attributes currently exist.', 'jigoshop') ?></td></tr><?php
						endif;
						?>
						</tbody>
					</table>
		...
		<script type="text/javascript">
			...
			jQuery('a.delete').click(function(){
				return confirm("<?php _e('Are you sure you want to delete this?', 'jigoshop'); ?>");
			});
			/* ]]> */
		</script>
	</div>
