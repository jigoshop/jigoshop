...
class Jigoshop_Report_Sales_By_Category extends Jigoshop_Admin_Report
{
	...
	public function get_products_in_category($category_id)
	{
		...
		return array_unique(apply_filters('jigoshop_report_sales_by_category_get_products_in_category', $product_ids, $category_id));
	}
	...
	public function category_widget()
	{
		...
		<form method="GET">
			<div>
				<select multiple="multiple" data-placeholder="<?php _e('Select categories&hellip;', 'jigoshop'); ?>" class="jigoshop-enhanced-select" id="show_categories" name="show_categories[]" style="width: 205px;">
					<?php
					$r = array();
					$r['pad_counts'] = 1;
					$r['hierarchical'] = 1;
	...
	public function get_export_button()
	{
		...
		?>
		<a href="#" download="report-<?php echo esc_attr( $current_range ); ?>-<?php echo date_i18n( 'Y-m-d', current_time('timestamp') ); ?>.csv" class="export_csv export_link" data-export="chart" data-xaxes="<?php _e( 'Date', 'jigoshop' ); ?>" data-groupby="<?php echo $this->chart_groupby; ?>">
	...
	public function get_main_chart()
	{
		...
		if (!$this->show_categories) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php _e( '&larr; Choose a category to view stats', 'jigoshop' ); ?></p>
			</div>
			<?php
		} else {
			$chart_data = array();
							...
							<?php
								...
								foreach ( $chart_data as $data ) {
									...
									echo '{
										...
										data: $.parseJSON( "' . json_encode( $series ) . '" ),
										color: "' . $color . '",
										bars: {
											fillColor: "' . $color . '",
											fill: true,
