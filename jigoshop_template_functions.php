...
if (!function_exists('jigoshop_show_product_sale_flash')) {
	function jigoshop_show_product_sale_flash($post, $_product)
	{
		if ($_product->is_on_sale()) {
			echo '<span class="onsale">'.__('Sale!', 'jigoshop').'</span>';
		}
	}
}
...
if (!function_exists('jigoshop_template_single_excerpt')) {
	function jigoshop_template_single_excerpt($post, $_product)
	{
		if ($post->post_excerpt) {
			echo apply_filters('jigoshop_single_product_excerpt', wpautop(wptexturize($post->post_excerpt)));
		}
	}
}
...
if (!function_exists('jigoshop_template_single_meta')) {
	function jigoshop_template_single_meta($post, $_product)
	{
		...
		if ($options->get('jigoshop_enable_sku') == 'yes' && !empty($_product->sku)) :
			echo '<div class="sku">'.__('SKU', 'jigoshop').': '.$_product->sku.'</div>';
		endif;
		if ($options->get('jigoshop_enable_brand') == 'yes' && !empty($_product->brand)) :
			echo '<div class="brand">'.__('Brand', 'jigoshop').': '.$_product->brand.'</div>';
		endif;
		if ($options->get('jigoshop_enable_gtin') == 'yes' && !empty($_product->gtin)) :
			echo '<div class="gtin">'.__('GTIN', 'jigoshop').': '.$_product->gtin.'</div>';
		endif;
		if ($options->get('jigoshop_enable_mpn') == 'yes' && !empty($_product->mpn)) :
...
if (!function_exists('jigoshop_template_single_sharing')) {
	function jigoshop_template_single_sharing($post, $_product)
	{
		...
		if ($jigoshop_options->get('jigoshop_sharethis')) :
...
if (!function_exists('jigoshop_product_description_tab')) {
	function jigoshop_product_description_tab($current_tab)
	{
		...
		<li <?php if ($current_tab == '#tab-description') {
			echo 'class="active"';
		} ?>><a href="#tab-description"><?php _e('Description', 'jigoshop'); ?></a></li>
	<?php
	}
}
if (!function_exists('jigoshop_product_attributes_tab')) {
	function jigoshop_product_attributes_tab($current_tab)
	{
		...
		<li <?php if ($current_tab == '#tab-attributes') {
			echo 'class="active"';
		} ?>><a href="#tab-attributes"><?php _e('Additional Information', 'jigoshop'); ?></a>
			</li>
		<?php endif;
	}
}
	...
	function jigoshop_product_reviews_tab($current_tab)
	{
		if (comments_open()) : ?>
			<li <?php if ($current_tab == '#tab-reviews') {
				...
				href="#tab-reviews"><?php _e('Reviews', 'jigoshop'); ?><?php echo comments_number(' (0)', ' (1)', ' (%)'); ?></a>
			</li>
		<?php endif;
	}
}
	...
	function jigoshop_product_customize_tab($current_tab)
	{
		...
		if (get_post_meta($_product->ID, 'customizable', true) == 'yes') {
			...
			<li <?php if ($current_tab == '#tab-customize') {
				echo 'class="active"';
			} ?>><a href="#tab-customize"><?php _e('Personalize', 'jigoshop'); ?></a></li>
		<?php
		}
	}
}
	...
	function jigoshop_product_customize_panel()
	{
		...
		if (get_post_meta($_product->ID, 'customizable', true) == 'yes') :
			...
			<form action="#" method="post">
				...
				if ($custom_length == '') :
					?>
					<textarea id="jigoshop_customized_product" name="jigoshop_customized_product" cols="60" rows="4"><?php echo esc_textarea($custom); ?></textarea>
				<?php else : ?>
					<input type="text" id="jigoshop_customized_product" name="jigoshop_customized_product" size="<?php echo $custom_length; ?>" maxlength="<?php echo $custom_length; ?>" value="<?php echo esc_attr($custom); ?>" />
				<?php endif; ?>
...
if (!function_exists('jigoshop_product_cat_image')) {
	function jigoshop_product_cat_image($id)
	{
		...
		$category_image = $thumbnail_id ? wp_get_attachment_url($thumbnail_id) : JIGOSHOP_URL.'/assets/images/placeholder.png';
...
if (!function_exists('jigoshop_output_related_products')) {
	...
	{
		$options = Jigoshop_Base::get_options();
		if ($options->get('jigoshop_enable_related_products') != 'no') // 2 Related Products in 2 columns
		{
		...
		if (jigoshop_shipping::show_shipping_calculator()) :
			...
			<form class="shipping_calculator" action="<?php echo esc_url(jigoshop_cart::get_cart_url()); ?>" method="post">
				...
				<section class="shipping-calculator-form">
						...
						<p class="form-row col-1">
							...
							if (jigoshop_countries::country_has_states($current_cc)) :
								...
								<select name="calc_shipping_state" id="calc_shipping_state">
									<option value=""><?php _e('Select a state&hellip;', 'jigoshop'); ?></option><?php
					...
					<p>
						<button type="submit" name="calc_shipping" value="1" class="button"><?php _e('Update Totals', 'jigoshop'); ?></button>
					</p>
					<p>
						<?php
...
if (!function_exists('jigoshop_checkout_login_form')) {
	...
	{
		$options = Jigoshop_Base::get_options();
		if (is_user_logged_in() || $options->get('jigoshop_enable_guest_login') != 'yes') {
			return;
		}
...
if (!function_exists('jigoshop_verify_checkout_states_for_countries_message')) {
	...
	{
		if (Jigoshop_Base::get_options()->get('jigoshop_verify_checkout_info_message') == 'yes') {
...
if (!function_exists('jigoshop_eu_b2b_vat_message')) {
	function jigoshop_eu_b2b_vat_message()
	{
		if (jigoshop_countries::is_eu_country(jigoshop_customer::get_country())
			&& Jigoshop_Base::get_options()->get('jigoshop_eu_vat_reduction_message') == 'yes'
		) {
