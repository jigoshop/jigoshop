...
function jigoshop_add_order_item() {
	...
	<tr class="item">
        ...
        <td class="cost"><input type="text" name="item_cost[]" placeholder="<?php _e('Cost per unit ex. tax e.g. 2.99', 'jigoshop'); ?>" value="<?php echo esc_attr( Jigoshop_Options::instance()->get_option('jigoshop_prices_include_tax') == 'yes' ? $_product->get_price_excluding_tax() : $_product->get_price() ); ?>" /></td>
 ...
 * When default permalinks are enabled, redirect shop page to post type archive url
 **/
if (Jigoshop_Options::instance()->get_option( 'permalink_structure' )=="") add_action( 'init', 'jigoshop_shop_page_archive_redirect' );
...
function jigoshop_add_to_cart_action($url = false)
{
    ...
    if ($product_added) {
    	...
    	switch ( Jigoshop_Options::instance()->get_option('jigoshop_redirect_add_to_cart', 'same_page') ) {
    		case 'same_page':
    ...
    else if (Jigoshop_Options::instance()->get_option('jigoshop_redirect_add_to_cart', 'same_page') == 'to_checkout' && jigoshop::error_count() == 0) {
        wp_safe_redirect(jigoshop_cart::get_checkout_url());
    }
    // Redirect directly to cart if no error messages
    else if (Jigoshop_Options::instance()->get_option('jigoshop_redirect_add_to_cart', 'to_cart') == 'to_cart' && jigoshop::error_count() == 0) {
...
function jigoshop_ga_tracking() {
	...
	$tracking_id = Jigoshop_Options::instance()->get_option('jigoshop_ga_id');
...
function jigoshop_ga_ecommerce_tracking( $order_id ) {
	...
	// Skip if disabled
	if ( Jigoshop_Options::instance()->get_option('jigoshop_ga_ecommerce_tracking_enabled') != 'yes' )
		return false;
