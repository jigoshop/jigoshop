...
function jigoshop_add_to_cart_action($url = false)
{
    ...
    else if (Jigoshop_Options::instance()->get_option('jigoshop_redirect_add_to_cart', 'same_page') == 'to_checkout' && !jigoshop::has_errors()) {
        wp_safe_redirect(jigoshop_cart::get_checkout_url());
    }
    // Redirect directly to cart if no error messages
    else if (Jigoshop_Options::instance()->get_option('jigoshop_redirect_add_to_cart', 'to_cart') == 'to_cart' && !jigoshop::has_errors()) {
