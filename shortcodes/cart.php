	...
	<div class="cart-collaterals">
		...
		if ($available_methods || !jigoshop_customer::get_shipping_country() || !jigoshop_shipping::is_enabled() ) :
			...
			<table cellspacing="0" cellpadding="0">
				<tbody>
					<tr>
						<td class="totals-subtotal-title"><?php _e('Subtotal', 'jigoshop'); ?></td>
						<td class="totals-subtotal"><?php echo jigoshop_cart::get_cart_subtotal(); ?></td>
					</tr>

					<?php if (jigoshop_cart::get_cart_shipping_total()) : ?><tr>
						<td class="totals-shipping-title"><?php _e('Shipping', 'jigoshop'); ?> <small><?php echo jigoshop_countries::shipping_to_prefix().' '.jigoshop_countries::$countries[ jigoshop_customer::get_shipping_country() ]; ?></small></td>
						<td class="totals-shipping"><?php echo jigoshop_cart::get_cart_shipping_total(); ?> <small><?php echo jigoshop_cart::get_cart_shipping_title(); ?></small></td>
					</tr><?php endif; ?>
					<?php if (jigoshop_cart::get_cart_tax()) : ?><tr>
						<td class="totals-tax-title"><?php _e('Tax', 'jigoshop'); ?> <?php if (jigoshop_customer::is_customer_outside_base()) : ?><small><?php echo sprintf(__('estimated for %s', 'jigoshop'), jigoshop_countries::estimated_for_prefix() . jigoshop_countries::$countries[ jigoshop_countries::get_base_country() ] ); ?></small><?php endif; ?></td>
						<td class="totals-tax"><?php
							echo jigoshop_cart::get_cart_tax();
						?></td>
					</tr><?php endif; ?>

					<?php if (jigoshop_cart::get_total_discount()) : ?><tr class="discount">
						<td class="totals-discount-title"><?php _e('Discount', 'jigoshop'); ?></td>
						<td class="totals-discount">-<?php echo jigoshop_cart::get_total_discount(); ?></td>
					</tr><?php endif; ?>
					<tr>
						<td class="totals-total-title"><strong><?php _e('Total', 'jigoshop'); ?></strong></td>
						<td class="totals-total"><strong><?php echo jigoshop_cart::get_total(); ?></strong></td>
					</tr>
				</tbody>
			</table>
