    ...
    if (is_user_logged_in()) :
        ...
        if ($jigoshop_orders->orders)
            foreach ($jigoshop_orders->orders as $order) :
                ...
                if ($order->formatted_shipping_address)
                    echo $order->formatted_shipping_address; else
                    echo '&ndash;';
                ?></address></td>
                            <td><?php echo jigoshop_price($order->order_total); ?></td>
                            <td><?php echo $order->status; ?></td>
        ...
        <div class="col2-set addresses">
            ...
            <div class="col-1">
                ...
                <address>
                    <?php
                    if (isset(jigoshop_countries::$countries->countries[get_user_meta(get_current_user_id(), 'billing-country', true)]))
                        $country = jigoshop_countries::$countries->countries[get_user_meta(get_current_user_id(), 'billing-country', true)]; else
                        $country = '';
                    $address = array(
                        get_user_meta(get_current_user_id(), 'billing-first_name', true) . ' ' . get_user_meta(get_current_user_id(), 'billing-last_name', true)
                        , get_user_meta(get_current_user_id(), 'billing-company', true)
                    ...
                    foreach ($address as $part)
                        if (!empty($part))
                            $formatted_address[] = $part;
                    $formatted_address = implode(', ', $formatted_address);
                    if (!$formatted_address)
                        _e('You have not set up a billing address yet.', 'jigoshop'); else
            ...
            <div class="col-2">
                ...
                <address>
                    <?php
                    if (isset(jigoshop_countries::$countries->countries[get_user_meta(get_current_user_id(), 'shipping-country', true)]))
                        $country = jigoshop_countries::$countries->countries[get_user_meta(get_current_user_id(), 'shipping-country', true)]; else
                        $country = '';
                    $address = array(
                        get_user_meta(get_current_user_id(), 'shipping-first_name', true) . ' ' . get_user_meta(get_current_user_id(), 'shipping-last_name', true)
                        , get_user_meta(get_current_user_id(), 'shipping-company', true)
                    ...
                    $formatted_address = implode(', ', $formatted_address);
                    if (!$formatted_address)
                        _e('You have not set up a shipping address yet.', 'jigoshop'); else
                        echo $formatted_address;
                    ?>
                </address>
...
function jigoshop_edit_address() {
    ...
    if (is_user_logged_in()) :
        ...
        if (isset($_GET['address']))
            $load_address = $_GET['address']; else
            $load_address = 'billing';
        if ($load_address == 'billing')
            $load_address = 'billing'; else
        ...
        <form action="<?php echo add_query_arg('address', $load_address, get_permalink(get_option('jigoshop_edit_address_page_id'))); ?>" method="post">
            ...
            <h3><?php
        if ($load_address == 'billing')
            _e('Billing Address', 'jigoshop'); else
            ...
            <p class="form-row form-row-first">
                ...
                <select name="address-country" id="address-country" class="country_to_state" rel="address-state">
                    ...
                    foreach (jigoshop_countries::$countries as $key => $value) :
                        ...
                        elseif (!$address['country'] && jigoshop_customer::get_country() == $key)
                            echo 'selected="selected"';
                        echo '>' . $value . '</option>';
                    endforeach;
                    ?>
