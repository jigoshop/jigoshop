...
<div class="wrap jigoshop">
	<div class="icon32 icon32-jigoshop-reports" id="icon-jigoshop"><br /></div>
	<h2 class="nav-tab-wrapper">
		<?php
		foreach ($reports as $key => $report_group) {
			echo '<a href="'.admin_url('admin.php?page=jigoshop_reports&tab='.urlencode($key)).'" class="nav-tab ';
		...
		?>
		<?php do_action('jigoshop_reports_tabs'); ?>
	</h2>
		...
		<ul class="subsubsub">
			<li><?php
				...
				foreach ($reports[$current_tab]['reports'] as $key => $report) {
					$link = '<a href="admin.php?page=jigoshop_reports&tab='.urlencode($current_tab).'&amp;report='.urlencode($key).'" class="';
