	...
	<li class="product <?php if ($loop%$columns==0) echo 'last'; if (($loop-1)%$columns==0) echo 'first'; ?>">
		...
		<?php 
			...
			if ( has_post_thumbnail() ) 
				the_post_thumbnail('shop_small'); 
			else 
				echo '<img src="'.jigoshop::plugin_url(). '/assets/images/placeholder.png" alt="Placeholder" width="'.jigoshop::get_var('shop_small_w').'" height="'.jigoshop::get_var('shop_small_h').'" />'; 
		?>
			<strong><?php the_title(); ?></strong>
			<span class="price"><?php echo $_product->get_price_html(); ?></span>
		</a>
	
		<?php do_action( 'jigoshop_template_loop_shop_before_button' ); ?>
