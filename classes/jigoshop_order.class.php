...
class jigoshop_order {
	...
	function email_bank_details() {
		...
		$title 			= self::get_jigoshop_options()->get_option('jigoshop_bank_transfer_title');
		$description 	= self::get_jigoshop_options()->get_option('jigoshop_bank_transfer_description');
		$bank_name 		= self::get_jigoshop_options()->get_option('jigoshop_bank_transfer_bank_name');
		$acc_number 	= self::get_jigoshop_options()->get_option('jigoshop_bank_transfer_acc_number');
		$sort_code 		= self::get_jigoshop_options()->get_option('jigoshop_bank_transfer_sort_code');
		$iban 			= self::get_jigoshop_options()->get_option('jigoshop_bank_transfer_iban');
		$bic 			= self::get_jigoshop_options()->get_option('jigoshop_bank_transfer_bic');
		$additional 	= self::get_jigoshop_options()->get_option('jigoshop_bank_transfer_additional');
	...
	function get_checkout_payment_url() {
		...
		if (self::get_jigoshop_options()->get_option('jigoshop_force_ssl_checkout')=='yes' || is_ssl()) $payment_page = str_replace('http:', 'https:', $payment_page);
		...
		if (sizeof($this->items)>0) foreach ($this->items as $item) :
			...
			if ($item['id']>0) :
				...
				if ( $_product->exists && $_product->managing_stock() ) :
                    ...
                    if (self::get_jigoshop_options()->get_option('jigoshop_notify_no_stock_amount') >= 0 && self::get_jigoshop_options()->get_option('jigoshop_notify_no_stock_amount') >= $new_quantity) :
						do_action('jigoshop_no_stock_notification', $item['id']);
					elseif (self::get_jigoshop_options()->get_option('jigoshop_notify_low_stock_amount') && self::get_jigoshop_options()->get_option('jigoshop_notify_low_stock_amount')>=$new_quantity) :
