...
class jigoshop_tax {
    ...
    function get_tax_classes() {
        $classes = Jigoshop_Options::instance()->get_option( 'jigoshop_tax_classes' );
    ...
    public function update_tax_amount($tax_class, $amount, $recalculate_tax = true) {
        ...
        if ($tax_class) :
            ...
            if ($recalculate_tax) :
                $rate = $this->get_rate($tax_class);
                $tax = $this->calc_tax($amount, $rate, Jigoshop_Options::instance()->get_option( 'jigoshop_prices_include_tax' ) == 'yes');
