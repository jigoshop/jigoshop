...
class jigoshop_cart extends Jigoshop_Singleton {
    ...
    function add_to_cart($product_id, $quantity = 1, $variation_id = '', $variation = array()) {
        ...
        if ($product->managing_stock() && !$product->has_enough_stock($quantity + $in_cart_qty)) :
            if ($in_cart_qty > 0) :
				$error = ($this->jigoshop_options->get_option('jigoshop_show_stock') == 'yes') ? sprintf(__('We are sorry.  We do not have enough "%s" to fill your request.  You have %d of them in your Cart and we have %d available at this time.', 'jigoshop'), $product->get_title(), $in_cart_qty, $product->get_stock()) : sprintf(__('We are sorry.  We do not have enough "%s" to fill your request.', 'jigoshop'), $product->get_title());
            else :
				$error = ($this->jigoshop_options->get_option('jigoshop_show_stock') == 'yes') ? sprintf(__('We are sorry.  We do not have enough "%s" to fill your request. There are only %d left in stock.', 'jigoshop'), $product->get_title(), $product->get_stock()) : sprintf(__('We are sorry.  We do not have enough "%s" to fill your request.', 'jigoshop'), $product->get_title());
			endif;
    ...
    function ship_to_billing_address_only() {
        return ($this->jigoshop_options->get_option('jigoshop_ship_to_billing_address_only') == 'yes');
    }
    ...
    private static function calculate_cart_total() {
        ...
        foreach (self::$cart_contents as $cart_item_key => $values) :
            ...
            if ($_product->exists() && $values['quantity'] > 0) :
                ...
                if ($this->jigoshop_options->get_option('jigoshop_calc_taxes') == 'yes') :
                    ...
                    if ($_product->is_taxable()) :
                        ...
                        if ($this->jigoshop_options->get_option('jigoshop_prices_include_tax') == 'yes' && jigoshop_customer::is_customer_outside_base(jigoshop_shipping::is_enabled() && $_product->requires_shipping()) && ($this->jigoshop_options->get_option('jigoshop_enable_shipping_calc')=='yes' ||  (defined('JIGOSHOP_CHECKOUT') && JIGOSHOP_CHECKOUT ))) :
                            ...
                            $price_includes_tax = ($this->jigoshop_options->get_option('jigoshop_tax_after_coupon') == 'yes' && ($cart_discount_amount > 0 || $current_product_discount > 0)? false : $this->jigoshop_options->get_option('jigoshop_prices_include_tax') == 'yes');
                            $tax_classes_applied = self::$tax->calculate_tax_amounts(($this->jigoshop_options->get_option('jigoshop_tax_after_coupon') == 'yes'  && ($cart_discount_amount > 0 || $current_product_discount > 0) ? $discounted_item_price * 100 : $total_item_price), $_product->get_tax_classes(), $price_includes_tax);
                            ...
                            if ($this->jigoshop_options->get_option('jigoshop_prices_include_tax') == 'yes' && $this->jigoshop_options->get_option('jigoshop_tax_after_coupon') == 'yes' && $discounted_item_price >= 0) :
    ...
    function calculate_totals() {
        ...
        // at the start
        if (self::$applied_coupons && $this->jigoshop_options->get_option('jigoshop_tax_after_coupon') == 'no') :
            foreach (self::$applied_coupons as $code) :
                if ($coupon = jigoshop_coupons::get_coupon($code)) :
        ...
        if ($this->jigoshop_options->get_option('jigoshop_calc_taxes') == 'yes' && !self::$tax->get_total_shipping_tax_amount()) :

            foreach (self::get_applied_tax_classes() as $tax_class) :
                if (!self::is_not_compounded_tax($tax_class)) : //tax compounded
                    $discount = ($this->jigoshop_options->get_option('jigoshop_tax_after_coupon') == 'yes' ? self::$discount_total : 0);
        ...
        if ($this->jigoshop_options->get_option('jigoshop_calc_taxes') == 'yes' && $this->jigoshop_options->get_option('jigoshop_display_totals_tax') == 'no' || ( defined('JIGOSHOP_CHECKOUT') && JIGOSHOP_CHECKOUT )) :
    ...
    public static function get_cart_subtotal($for_display = true, $apply_discount_and_shipping = false) {
        ...
        if ($this->jigoshop_options->get_option('jigoshop_calc_taxes') == 'no') :
            ...
            if ($this->jigoshop_options->get_option('jigoshop_display_totals_tax') == 'no' || ( defined('JIGOSHOP_CHECKOUT') && JIGOSHOP_CHECKOUT )) :
                ...
                if ($this->jigoshop_options->get_option('jigoshop_prices_include_tax') == 'yes') :
                    $subtotal = ($apply_discount_and_shipping && $this->jigoshop_options->get_option('jigoshop_tax_after_coupon') == 'yes' ? self::$subtotal_ex_tax + $discount + self::$shipping_total : self::$subtotal_ex_tax);
                else :
                    $subtotal = ($apply_discount_and_shipping && $this->jigoshop_options->get_option('jigoshop_tax_after_coupon') == 'yes' ? self::$subtotal + $discount + self::$shipping_total : self::$subtotal);
                endif;
                ...
                if ($this->jigoshop_options->get_option('jigoshop_prices_include_tax') == 'yes') :
                    ...
                    $subtotal = ($apply_discount_and_shipping && $this->jigoshop_options->get_option('jigoshop_tax_after_coupon') == 'yes' ? self::$subtotal + self::get_total_cart_tax_without_shipping_tax() + $discount + self::$shipping_total : self::$subtotal + self::get_total_cart_tax_without_shipping_tax());
                endif;
