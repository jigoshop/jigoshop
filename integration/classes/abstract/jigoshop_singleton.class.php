		...
		static function get_called_class($bt = false, $l = 2)
		{
			...
			if (!isset($bt[$l]['type'])) {
				...
				switch ($bt[$l]['type']) {
						...
						switch ($bt[$l]['function']) {
							case '__get':
								...
								if (!is_object($bt[$l]['object'])) {
									throw new Exception ("Could not find called class -> edge case fail. __get called on non object.");
								}
					...
					default:
						throw new Exception ("Could not find called class -> unknown backtrace method type");
				}
			}
		}
	}
