...
class dibs extends jigoshop_payment_gateway {
	...
	public function __construct() {
		...
		$this->key1 = Jigoshop_Base::get_options()->get_option('jigoshop_dibs_key1');
		$this->key2 = Jigoshop_Base::get_options()->get_option('jigoshop_dibs_key2');
	...
	public function generate_form( $order_id ) {
		...
		$args =
			array(
				...
				'currency'   => $dibs_currency[Jigoshop_Base::get_options()->get_option('jigoshop_currency')],
				'ordertext'  => 'TEST',
		...
		$args['md5key'] = MD5(Jigoshop_Base::get_options()->get_option('jigoshop_dibs_key2') . MD5(Jigoshop_Base::get_options()->get_option('jigoshop_dibs_key1') . 'merchant=' . $args['merchant'] . '&orderid=' . $args['orderid'] . '&currency=' . $args['currency'] . '&amount=' . $args['amount']));
	...
	**/
	function successful_request( $posted ) {
		...
		if ( !empty($posted['transact']) && !empty($posted['orderid']) && is_numeric($posted['orderid']) ) {
			...
			// Verify MD5 checksum
			// http://tech.dibs.dk/dibs_api/other_features/md5-key_control/
			$key1 = Jigoshop_Base::get_options()->get_option('jigoshop_dibs_key1');
			$key2 = Jigoshop_Base::get_options()->get_option('jigoshop_dibs_key2');
